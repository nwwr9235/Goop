...
# ✅ محاولة استيراد yt_dlp أو youtube_dl
try:
    ...
    try:
        import youtube_dl
        YT_DOWNLOADER = "youtube_dl"
    except ImportError:
        youtube_dl = None
        YT_DOWNLOADER = None
        logging.warning("⚠️ لا يوجد محمل فيديو مثبت (yt_dlp أو youtube_dl)")
...
try:
    ...
    # ✅ إذا كانت المكتبة الجديدة مثبتة
    try:
        ...
        PYTGCALLS_TYPE = None
        logging.warning("⚠️ لا يوجد pytgcalls مثبت")
...
# ✅ تهيئة PyTgCalls حسب النوع المثبت
group_call_factory = None
pytgcalls = None
active_group_calls = {}

if PYTGCALLS_TYPE == "old":
    group_call_factory = GroupCallFactory(app)
    logger.info("✅ تم تهيئة GroupCallFactory (المكتبة القديمة)")
elif PYTGCALLS_TYPE == "new":
    pytgcalls = PyTgCalls(app)
    logger.info("✅ تم تهيئة PyTgCalls (المكتبة الجديدة)")
...
async def download_song(query):
    ...
    if youtube_dl is None:
        logger.error("❌ لا يوجد محمل فيديو مثبت")
        return None
...
async def play_handler(client, message):
    ...
    if YT_DOWNLOADER is None:
        return await message.reply("❌ نظام التحميل غير متوفر. تأكد من تثبيت yt-dlp")
    
    if PYTGCALLS_TYPE is None:
        return await message.reply("❌ نظام المكالمات غير متوفر")
...
async def skip_handler(client, message):
    ...
    try:
        if PYTGCALLS_TYPE == "old" and chat_id in active_group_calls:
...
if __name__ == "__main__":
    ...
    logger.info(f"📞 نظام المكالمات: {PYTGCALLS_TYPE}")
